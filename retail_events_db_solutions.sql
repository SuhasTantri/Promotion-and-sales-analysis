...
from cte) a
where r<=5;
-- Analyse whether promotion campaign has been a success or not on the basis of the revenue generated by the company
select * from fact_events;
...
where r<=10; 
-- Performance of stores in terms of city
with cte as(select ds.city,round(sum(base_price * quantity_sold_before_promo)/1000000,1) as revenue_before_promo, 
	round(sum(case
		when promo_type IN ('50% OFF','BOGOF') then (base_price/2)*quantity_sold_after_promo
        when promo_type = '25% OFF' then (base_price/4)*quantity_sold_after_promo
        when promo_type = '33% OFF' then (base_price*(1-0.33))*quantity_sold_after_promo
...
on fe.store_id = ds.store_id
group by 1)
select city, ((revenue_after_promo/revenue_before_promo)-1)*100.0 as "IR%",
dense_rank() over(order by ((revenue_after_promo/revenue_before_promo)-1)*100.0 desc) as store_rank
from cte;
-- Stores in Madurai are performing the best
-- Promotion analysis
...
with cte as (select promo_type, sum(quantity_sold_after_promo-quantity_sold_before_promo) as Inc_sold_units,
dense_rank() over(order by sum(quantity_sold_after_promo-quantity_sold_before_promo)) as r from
fact_events
group by promo_type)
select * from cte
where r<=2;
...
from fact_events fe
group by promo_type),
cte_2 as (select promo_type, (sum(quantity_sold_after_promo)/ sum(quantity_sold_before_promo)-1)*100 as 'IC%',
dense_rank() over(order by (sum(quantity_sold_after_promo)/ sum(quantity_sold_before_promo)-1)desc) as re from
fact_events
group by promo_type)
select cte_2.*,((revenue_after_promo/revenue_before_promo)-1)*100 as 'IR%',
dense_rank() over(order by ((revenue_after_promo/revenue_before_promo)-1) desc) as r
...
select dp.category,(sum(quantity_sold_after_promo)/sum(quantity_sold_before_promo)-1)*100 as 'ISU%',
dense_rank() over(order by (sum(quantity_sold_after_promo)/sum(quantity_sold_before_promo)-1)*100 desc) as re from
fact_events f
join dim_products dp on
dp.product_code = f.product_code
group by 1;
/*Home Appliances category units sold increased significantly due to promotions
Personal care catgory did not have significant impact because of promotions*/
-- Incremental revenue % based on promotion type
with cte as (select promo_type,round(sum(base_price * quantity_sold_before_promo)/1000000,1) as revenue_before_promo, 
	round(sum(case
		when promo_type IN ('50% OFF','BOGOF') then (base_price/2)*quantity_sold_after_promo
        when promo_type = '25% OFF' then (base_price/4)*quantity_sold_after_promo
...
from fact_events fe
group by promo_type)
select *,((revenue_after_promo/revenue_before_promo)-1)*100 as 'IR%',
dense_rank() over(order by ((revenue_after_promo/revenue_before_promo)-1)*100 desc) as r
from cte;
